...
1. **Docker vs Native Setup**: อธิบายข้อดีของการใช้ Docker เปรียบเทียบกับการติดตั้ง ESP-IDF บน host system
  ...
  Docker: ใช้งานง่าย ติดตั้งครั้งเดียว ใช้ได้ทุกเครื่อง ไม่กระทบระบบหลัก
  Native: เร็วกว่าเล็กน้อย แต่ติดตั้งซับซ้อน และขึ้นกับระบบ

2. **Build Process**: อธิบายขั้นตอนการ build ของ ESP-IDF ใน Docker container ตั้งแต่ source code จนได้ binary

  รัน container ด้วย docker-compose up
  เข้า container → docker-compose exec esp32-dev bash
  รัน idf.py build
  สร้างระบบ build ด้วย CMake
  Compile ด้วย Ninja → ได้ไฟล์ .bin, .elf ในโฟลเดอร์ build/

3. **CMake Files**: บทบาทของไฟล์ CMakeLists.txt แต่ละไฟล์คืออะไร และทำงานอย่างไรใน Docker environment?
 
  /CMakeLists.txt: กำหนดโปรเจกต์หลัก
  /main/CMakeLists.txt: ระบุ source code และ component
  ทำงานร่วมกับ ESP-IDF ผ่าน idf.py ใน container

4. **Git Ignore**: ไฟล์ .gitignore มีความสำคัญอย่างไรสำหรับ ESP32 project development?
 
  กันไฟล์ไม่จำเป็นเข้า Git เช่น build/, .venv/
  ลดขนาด repo และป้องกันไฟล์ไม่เสถียร

5. **Container Persistence**: ข้อมูลใดบ้างที่จะหายไปเมื่อ restart container และข้อมูลใดที่จะอยู่ต่อ?
 
  ข้อมูลที่จะหาย: ไฟล์ใน container ถ้าไม่ได้ mount
  ข้อมูลที่อยู่ต่อ: ไฟล์ในโฟลเดอร์ที่ mount (เช่น /project), volumes

6. **Development Workflow**: เปรียบเทียบ workflow การพัฒนาระหว่างการใช้ Docker กับการทำงานบน native system
   
  Docker:ตั้งค่าง่าย แค่ใช้ container ที่เตรียมไว้ ไม่ต้องติดตั้งอะไรเพิ่ม สภาพแวดล้อมเหมือนกันทุกเครื่อง แต่ถ้าจะใช้ฮาร์ดแวร์จริง ต้องตั้งค่าเชื่อมต่อเพิ่ม
  Native system:ต้องติดตั้งเครื่องมือเองตั้งแต่ต้น ใช้เวลานานกว่า แต่เข้าถึงฮาร์ดแวร์ได้โดยตรง และทำงานกับอุปกรณ์จริงได้สะดวกกว่า
  สรุปคือ Docker เน้นความรวดเร็วและความเสถียรของสภาพแวดล้อม ส่วน Native เน้นการเข้าถึงฮาร์ดแวร์และประสิทธิภาพโดยตรง
